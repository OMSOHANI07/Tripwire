...
export function scoreClass(n: number): string {
  ...
  if (n >= 65) return "bg-amber-300 text-stone-900";
...
export function DecisionBanner({ trip, people }: { trip: TripPublic; people: string[] }) {
  ...
  if (!d) return null;
  return (
    <div className="rounded-2xl bg-teal-800 p-5 text-white shadow-md" role="status">
      <p className="text-sm font-semibold uppercase tracking-wide text-teal-100">Trip decided 🎉</p>
      <p className="mt-1 text-3xl font-bold">{d.name}</p>
      <p className="mt-1 text-teal-50">
        {d.start && d.end ? formatRange(d.start, d.end) : "Dates to confirm"}
        {d.cost ? ` · about ${inr(d.cost)} per person` : ""}
      </p>
      <p className="mt-1 text-sm text-teal-100">Going: {listNames(people.length ? people : trip.participants.map((p) => p.name))}</p>
      <div className="mt-4">
        <CopyButton
          text={summaryMessage(trip, people)}
          label="Copy summary for WhatsApp"
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 font-semibold text-teal-900 hover:bg-teal-50"
        />
      </div>
    </div>
  );
...
function OptionCard({
  ...
  return (
    <Card className={`space-y-4 ${isWinner ? "ring-2 ring-teal-700" : ""}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Option {String.fromCharCode(64 + rank)}</p>
          <h3 className="text-xl font-bold text-stone-900">{o.name}</h3>
          <p className="text-sm text-stone-600">{o.state}</p>
        </div>
        <div className="text-right">
          <p className="text-3xl font-bold text-teal-800">{o.groupScore}</p>
          <p className="text-xs text-stone-500">group score</p>
        </div>
      </div>
      ...
      <dl className="grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-xl bg-stone-50 p-3">
          <dt className="text-stone-500">Best dates</dt>
          <dd className="font-semibold text-stone-900">{o.window ? formatRange(o.window.start, o.window.end) : "No shared window"}</dd>
        </div>
        <div className="rounded-xl bg-stone-50 p-3">
          <dt className="text-stone-500">Est. cost / person</dt>
          <dd className="font-semibold text-stone-900">{inr(o.cost)}</dd>
          <dd className="text-xs text-stone-500">{inr(o.costMin)}–{inr(o.costMax)}, excl. travel</dd>
        </div>
      </dl>
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm leading-relaxed text-stone-700">{explanation?.why}</p>
        {explanation?.compromise && <p className="text-sm text-stone-700"><strong>Who compromises most:</strong> {explanation.compromise}</p>}
      </div>

      {o.tradeOffs.length > 0 && (
        <div>
          <p className="text-sm font-semibold text-stone-900">Trade-offs</p>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-stone-100 pt-3">
        <p className="text-sm text-stone-600">{plural(votes, "vote")}{isWinner && votes > 0 ? " · leading" : ""}</p>
        {canVote && (
          <button type="button" onClick={onVote} disabled={voting || myVote} aria-pressed={myVote} className={myVote ? btn.small + " border-teal-700 bg-teal-50 text-teal-900" : btn.small}>
...
export function StandsGrid({ options, people }: { options: OptionView[]; people: string[] }) {
  return (
    <Card>
      ...
      <div className="mt-3 flex flex-wrap gap-3 text-xs text-stone-600" aria-hidden>
...
function DateNotes({ notes }: { notes: DateNote[] }) {
  ...
  return (
    <Card>
      <h2 className="font-semibold text-stone-900">Date notes</h2>
    ...
    <div className="space-y-5">
      ...
      {data.status === "ok" && (
        <>
          ...
          {!trip.decided && votes && (
            <Card className="space-y-2">
              <h2 className="font-semibold text-stone-900">Final decision: vote for one</h2>
              ...
              {!me && (
                <p className="text-sm text-stone-500">To vote, open your personal link (the one you got after submitting) on this device.</p>
              )}
              {me && eligible && <p className="text-sm text-stone-700">Voting as <strong>{me.participant.name}</strong>.</p>}
      ...
      {(data.filteredOut?.length ?? 0) > 0 && (
        <Card>
          <details>
            <summary className="cursor-pointer font-semibold text-stone-900">Why not…? (strong options that got filtered out)</summary>
